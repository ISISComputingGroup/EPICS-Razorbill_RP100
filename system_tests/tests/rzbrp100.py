...
class Rzbrp100Tests(unittest.TestCase):
    ...
    def test_WHEN_identity_requested_THEN_correct_identity_returned(self):
        expected_id_string = "EMULATED Razorbill RP100 Strain Cell PSU"[0:39]    # 40 character PV field limit
    ...
    def test_WHEN_output_state_is_set_THEN_setpoint_readback_updates(self, _, chan, val):
        self.ca.assert_setting_setpoint_sets_readback(
            val, set_point_pv="CH{}:OUTPUT:SP".format(chan), readback_pv="CH{}:OUTPUT:SP:RBV".format(chan))
    ...
    def test_WHEN_voltage_setpoint_is_set_THEN_setpoint_readback_updates(self, _, chan, val):
        self.ca.assert_setting_setpoint_sets_readback(
            val, set_point_pv="CH{}:VOLT:SP".format(chan), readback_pv="CH{}:VOLT:SP:RBV".format(chan))

    @parameterized.expand(parameterized_list(itertools.product(CHANNELS, TEST_FLOAT_VALUES)))
    def test_WHEN_voltage_slewrate_setpoint_is_set_THEN_value_updates(self, _, chan, val):
        self.ca.assert_setting_setpoint_sets_readback(
            val, set_point_pv="CH{}:VOLT:SLEWRATE:SP".format(chan), readback_pv="CH{}:VOLT:SLEWRATE".format(chan))

    @parameterized.expand(parameterized_list(itertools.product(CHANNELS, TEST_FLOAT_VALUES)))
    def test_WHEN_voltage_slewrate_setpoint_is_set_THEN_setpoint_readback_updates(self, _, chan, val):
        self.ca.assert_setting_setpoint_sets_readback(
            val, set_point_pv="CH{}:VOLT:SLEWRATE:SP".format(chan), readback_pv="CH{}:VOLT:SLEWRATE:SP:RBV".format(chan))
